...
class Email:
    ...
    def register(self, auth_event, request):
        ...
        # user has been created/registered, appropiate permissions have been
        # set and it is an active user so now we send him an email 
    ...
    def generate_auth_code(self, auth_event, request):
        ...
        if (
            ...
            LOGGER.error(
                "Email.generate_auth_code error\n" +
        ...
        if not verify_num_successful_logins(auth_event, 'Email', user, req_data):
            LOGGER.error(
                "Email.generate_auth_code error\n" +
